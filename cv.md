...
- **sans_bass — in-browser stem separation & practice player** — A browser-only multitrack player for learning an instrument part by ear. Runs six-stem source separation entirely client-side (onnxruntime-web + htdemucs_6s on WebGPU at 8.4x realtime — 23.9 s for a 200 s track), then solos one instrument against an A–B loop driven on the audio thread so all six stems stay sample-locked. The audio never leaves the machine: no upload, no server, no build step. Output is held to the native Demucs pipeline by a parity harness (0.98–0.997 correlation per stem), backed by 81 dependency-free unit tests, zh-TW/English i18n, cookieless analytics, and per-PR preview deploys. Shipped v1.0 → v1.8 in nine days. https://sansword.github.io/sans_bass/
...
- **sans_enigma — cracking Enigma with Claude (from 610,000 years to 4.4 seconds)** — I picked the sources — a Computerphile video, then the 1995 Cryptologia paper behind it — and Claude Code turned each one into Python attacks on an Enigma M3 cipher. Every step is measured on the same laptop: a known-plaintext crib attack in 4.4 min, and a ciphertext-only staged attack (Gillogly's 52-trial ring recovery plus a trigram-scored plugboard search) in 4.4 s, against ~4.1 h for plugboard-free brute force and ~610,000 years with a plugboard. Re-measuring every number caught an overclaim — the fast ring recovery isn't always letter-perfect — and token usage was measured from the session transcript (~58% of one Claude Pro 5-hour limit). Published as a bilingual poster (https://sansword.github.io/posts/enigma_cracking_with_claude.html, https://sansword.github.io/posts/enigma_cracking_with_claude_zh.html) with code and a prompt-by-prompt development journal. https://github.com/SansWord/sans_enigma
