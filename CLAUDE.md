...
| Path | Description |
|------|-------------|
| `~/Source/github/sans_schema/` ⭐ **(Jul 2026)** | **sans_schema** — semantic query gateway: clients send `{want, where}` (their own field names + a plain-English filter) against a backend whose schema they never see; the gateway semantically resolves fields, compiles the filter to a validated predicate AST (injection boundary — raw NL→SQL never executed), runs on Postgres, answers in the client's keys with an `interpreted` echo. LLM in the request path with production plumbing (two-part cache, prompt-cache layout, confidence gate + refusal, SQLi/prompt-injection review); de-risked by a multi-vendor spike (~100%, 52-case benchmark, 3 vendors / 9 models). Python/FastAPI on Fly.io; Next.js playground on Vercel — live: https://sans-schema-playground.vercel.app/ (+ `/slides.html` deck). **The strongest AI × data-infra convergence artifact.** |
| `~/Source/github/guasi-tw/app/` ⭐ **FLAGSHIP PRODUCT (Jun 2026)** | **guasi (我是)** — a *deployed, multi-user* identity-verification product (live at https://guasi.tw). Cross-link the social accounts you own so a banned account's survivors can vouch for a new one; anyone can publicly verify which accounts are the same person. Platform-independent (no API access — relies on publicly auditable posted content). Full production stack: Next.js 16 / React 19, Postgres (Neon) + Prisma, NextAuth, Vercel with per-PR preview deploys on isolated Neon branches, post-deploy smoke tests. Built with agentic Claude Code workflows — maintained `docs/` doc-tree, cost ledger (`docs/operating-costs.md`), devlog. **The strongest "ships real production product end-to-end" artifact.** Unpublished article drafts live in `app/articles/` (see below). |
...
| `~/Source/github/aitian/` ⭐ **COMMUNITY (Jul 2026)** | **AI展 (aitian)** — weekly online AI-application meetup (Tuesdays 6:00–7:30 pm PT) that SansWord co-organizes and moderates; members demo AI side projects, workflows, experiments. Live at https://aitian.dev (meetups: `/meetup.html`, crew: `/moderators.html`). He also built the bilingual portal end-to-end: vanilla HTML/CSS/JS, Markdown+YAML → JSON at deploy with CI validation, PR-based contribution flow, GitHub Pages + custom domain — shipped v0.1→v0.8 in ~4 days with Claude Code. Presented sans_schema at the opening session (2026-07-14). **The community-leadership artifact.** |
| `~/Source/github/sans_bass/` ⭐ **NEWEST (Aug 2026)** | **sans_bass** — browser-only multitrack player for learning an instrument part by ear: load a song, separate it into six stems, solo one, and drill it against an A–B loop. **Runs real ML inference client-side** — `onnxruntime-web` + `htdemucs_6s` on WebGPU at **8.4× realtime** (23.9 s for a 200 s track); the audio never leaves the machine (no upload, no server, no build step). Output held to the native Demucs pipeline by a **parity harness** (0.98–0.997 correlation per stem) — the measurement artifact here. Vanilla HTML/JS, 81 dependency-free unit tests, zh-TW/English i18n, cookieless analytics, per-PR GitHub Pages previews; v1.0→v1.8 in 9 days. Live: https://sansword.github.io/sans_bass/. **The on-device / edge-ML artifact — the only project that runs a model rather than calling one.** |
| `~/Source/github/enigma/` ⭐ **(Sep 2026)** | **sans_enigma** (GitHub: `SansWord/sans_enigma`) — Enigma M3 cracking toolkit built with Claude Code from a Computerphile video and Gillogly's 1995 *Cryptologia* paper: crib attack (4.4 min), staged ciphertext-only attack (4.4 s), simulator. Every timing re-measured one run at a time; brute force ~4.1 h (no plugboard) / ~610,000 years (with); token usage measured from the transcript (~58% of one Pro 5-hour limit). `docs/development-journal.md` is the prompt-by-prompt log. Public poster: https://sansword.github.io/posts/enigma_cracking_with_claude.html (繁中: https://sansword.github.io/posts/enigma_cracking_with_claude_zh.html). **The "curate the sources, measure every claim" artifact.** |
